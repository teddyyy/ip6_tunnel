...
static void print_usage(FILE *f)
{
	...
	fprintf(f, "          [ mode { ip6ip6 | ipip6 | skinny | any } ]\n");
...
get_failed:
		...
		if (iptuninfo[IFLA_IPTUN_COLLECT_METADATA])
			metadata = 1;
		if (iptuninfo[IFLA_IPTUN_SKINNY])
			is_skinny = true;
		else
			is_skinny = false;
	}
	...
	if (is_skinny)
                addattr_l(n, 1024, IFLA_IPTUN_SKINNY, NULL, 0);
...
static void ip6tunnel_print_opt(struct link_util *lu, FILE *f, struct rtattr *tb[])
{
	...
	if (tb[IFLA_IPTUN_SKINNY])
		fprintf(f, "skinny mode is enabled ");
	else
		fprintf(f, "skinny mode is disabled ");
}
